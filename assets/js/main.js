		...
		function setupDesktop() {
			...
			function updatePreviewTransform(li, preview, x, y) {
				...
				if (imgRect.width === 0 || imgRect.height === 0) {
					// Layout not ready yet; retry after next frame so we set position before user sees a flash
					requestAnimationFrame(() => {
						requestAnimationFrame(() => updatePreviewTransform(li, preview, x, y));
					});
					return;
				}
				...
				preview.classList.add('artist__preview--positioned');
			}
			...
			function onLeave(e) {
				...
				if (!li) return;
				const pair = itemsWithPreview.find(({ li: l }) => l === li);
				if (pair) pair.preview.classList.remove('artist__preview--positioned');
			}

			rosterEl.addEventListener('mousemove', onMove, { passive: true });
			rosterEl.addEventListener('mouseleave', onLeave, { passive: true });
			teardownDesktop = () => {
				rosterEl.removeEventListener('mousemove', onMove);
				rosterEl.removeEventListener('mouseleave', onLeave);
				itemsWithPreview.forEach(({ preview }) => preview.classList.remove('artist__preview--positioned'));
			};
		}
	...
	function initCustomCursor() {
		...
		function setupCursor() {
			...
			function isPointerOverHoverTarget(x, y) {
				...
				return el ? !!el.closest(HOVER_SELECTOR) : false;
			}
			...
			function onMouseMove(e) {
				mouseX = e.clientX;
				mouseY = e.clientY;
				isHover = isPointerOverHoverTarget(mouseX, mouseY);
			...
			function tick() {
				...
				if (!hotspotSet) {
					/* Touch area / hotspot: start of top + horizontal centre of the SVG */
					hotspotX = cursorEl.offsetWidth * 0.5;
					hotspotY = 0;
					hotspotSet = true;
				}
				const x = mouseX - hotspotX;
				const y = mouseY - hotspotY;
			...
			document.addEventListener('visibilitychange', scheduleTick, { passive: true });
			scheduleTick();
